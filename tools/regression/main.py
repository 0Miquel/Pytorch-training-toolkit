...
def main(cfg):
    ...
    train_dataset = PollutionDataset(train=True, data_path=cfg.data_path, class_names=cfg.class_names,
                                     transforms=transforms_train, cams=cfg.cams, level=cfg.level)
    valid_dataset = PollutionDataset(train=False, data_path=cfg.data_path, class_names=cfg.class_names,
                                     transforms=transforms_val, cams=cfg.cams, level=cfg.level)
    ...
    # create the model
    model = Resnet(n_classes=cfg.n_classes, backbone=cfg.backbone)
