...
def mean_average_precision(
    ...
    average_precisions = torch.zeros((n_classes - 1), device=_pred_boxes.device, dtype=_pred_boxes.dtype)  # (n_classes - 1)
    for c in range(1, n_classes):
        ...
        # (n_class_objects)
        gt_class_boxes_detected = torch.zeros((gt_class_images.size(0)), dtype=torch.uint8, device=gt_class_images.device)
